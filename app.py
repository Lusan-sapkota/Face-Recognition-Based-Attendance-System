...
face_detector = cv2.CascadeClassifier('lusan_frontalface_default.xml')
...
# This function will run when we add a new user using the add new user form
...
def add():
    ...
    while True:
        ...
        if ret and frame is not None and frame.size != 0:
            ...
            for (x, y, w, h) in faces:
                cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
                if j % 10 == 0:
                    name = newusername + '_' + str(newuserid) + '_' + str(i) + '.jpg'
    ...
    train_model()
    names, rolls, times, l = extract_attendance()
    return render_template('home.html', names=names, rolls=rolls, times=times, l=l, totalreg=totalreg(), mess='User added successfully!')
